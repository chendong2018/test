...
import com.jianwu.domain.request.Pay;
import com.jianwu.domain.request.PayRequest;
...
import com.jianwu.utils.XStreamUtil;
 ...
 */
@RestController
@RequestMapping("/zip/wechat/pay")
public class WxPayController extends BaseController implements WxPayService {
    ...
    @PostMapping("/toPay")
    public ResultResponse toPay(PayRequest payRequest) throws WxPayException {
        WxPayConfig wxPayConfig = wxService.getConfig();
        WxPayUnifiedOrderRequest orderRequest = new WxPayUnifiedOrderRequest();
        orderRequest.setAppid(wxPayConfig.getAppId());
        orderRequest.setMchId(orderRequest.getMchId());
        orderRequest.setOpenid(payRequest.getOpenId());
        orderRequest.setOutTradeNo(payRequest.getOrderNo());
        orderRequest.setTotalFee((int) (payRequest.getPrice()*100));
        orderRequest.setBody(payRequest.getBody());
        orderRequest.setNotifyUrl(callbackUrl);
        orderRequest.setTradeType("JSAPI");
        orderRequest.setSpbillCreateIp(getUserIp());
        Pay pay = new Pay();
        WxPayMpOrderResult orderResult = this.wxService.createOrder(orderRequest);
        pay.setAppId(orderResult.getAppId());
        pay.setNonceStr(orderResult.getNonceStr());
        pay.setPackAge(orderResult.getPackageValue());
        pay.setTimeStamp(orderResult.getTimeStamp());
        pay.setSignType(orderResult.getSignType());
        pay.setPaySign(orderResult.getPaySign());
        return ResultResponse.success(pay);
    }

    @PostMapping(value = "/callback" )
    public WxPayRefundResult callback(  @RequestBody  String  wxPayRefundResults) {
        logger.info("wxPayRefundResults-->"+wxPayRefundResults);
        String xml = wxPayRefundResults.replace("xml","PayResult");
        logger.info("xml-->"+xml);
        String s = xml.substring(xml.indexOf("out_trade_no"),xml.lastIndexOf("out_trade_no"));
        String orderNumber = s.substring(s.lastIndexOf("[")+1,s.lastIndexOf("]")-1);
        s = xml.substring(xml.indexOf("return_code"),xml.lastIndexOf("return_code"));
        String returnCode = s.substring(s.lastIndexOf("[")+1,s.lastIndexOf("]")-1);
//        PayResult persons = new PayResult();
//        Object o = XStreamUtil.simplexml2object(xml,persons);
//        if(o == null){
//            return null;
//        }
//        String json = JSON.toJSONString(o);
//        System.out.println("--->"+json);
//        PayResult person = JSON.parseObject(json,PayResult.class);
//        logger.info("person----->{}",person.toString());
        if(returnCode.equals("SUCCESS")){
            //成功 则更新状态
            orderFormService.updateOrderForm(orderNumber, OrderStatus.TOPAID.getStatus());
        }
        WxPayRefundResult refundResult = new WxPayRefundResult();
        refundResult.setResultCode("SUCCESS");
        refundResult.setReturnMsg("OK");
        return refundResult;
    }
